...
def purchaseOrder(row):
    ...
    for x in range(pdCount):
        ...
        order_ws["B3"] = datetime.today().strftime("%Y%m%d") + "_" + str(orderCount)
        order_ws["B4"] = datetime.today().strftime("%Y-%m-%d")
        order_ws["A9"] = row[x]["supplier"] + " 귀하"

        # order_ws["A"+str(13+x)] = row[x]["supplier"]
        order_ws["B"+str(13+x)] = row[x]["product"]
        order_ws["C"+str(13+x)] = row[x]["inventory"]
        order_ws["D"+str(13+x)] = row[x]["lowInventory"]
...
for idx, row in enumerate(inventory_ws.rows):
    ...
    # print("업체: " + data["supplier"] + ", 재고단위: " + data["inventory"] + ", 제품: " + data["product"] + ", 부족재고: " + data["lowInventory"])
